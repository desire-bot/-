...
function updateAllTexts() {
    ...
    if (viewBtns.length >= 2) {
        viewBtns[0].textContent = t('body_view');
        viewBtns[1].textContent = t('organs_view');
    }
    ...
    const sectionTitles = document.querySelectorAll('#p-in .section-title');
    if (sectionTitles.length >= 2) {
        sectionTitles[1].textContent = t('symptom_title');
    }
    if (sectionTitles.length >= 3) {
        sectionTitles[2].textContent = t('intensity');
    }
...
function updateSelectedInfo() {
    ...
    if (!info) return;
    if (selZ && selOrgan) {
        info.textContent = `${t('selected')}: ${selZ} → ${selOrgan}`;
    } else if (selZ) {
        info.textContent = `${t('selected')}: ${selZ}`;
    } else {
        info.textContent = '';
    }
}
...
// ===== ПЕРЕКЛЮЧЕНИЕ ВИДОВ (Тело / Органы) =====
const viewBtns = document.querySelectorAll('.view-btn');
viewBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        const bodySilhouette = document.getElementById('silhouette-body');
        const organsSilhouette = document.getElementById('silhouette-organs');
        
        if (currentView === 'body') {
            bodySilhouette.style.display = 'flex';
            organsSilhouette.style.display = 'none';
        } else {
            bodySilhouette.style.display = 'none';
            organsSilhouette.style.display = 'flex';
        }
    });
});
...
// Зоны на теле (общий обработчик для всех SVG)
function setupZoneClick(selector) {
    ...
    elements.forEach(el => {
        el.addEventListener('click', (e) => {
            ...
            selZ = el.dataset.zone;
            
            // Если у элемента есть data-organ, используем его
            if (el.dataset.organ) {
                selOrgan = el.dataset.organ;
...
if (saveBtn) {
    saveBtn.addEventListener('click', () => {
        if (!selZ || !selT || !selI) {
            const missing = [];
            if (!selZ) missing.push(lang === 'ru' ? 'зона' : 'zone');
            if (!selT) missing.push(lang === 'ru' ? 'тип' : 'type');
            if (!selI) missing.push(lang === 'ru' ? 'интенсивность' : 'intensity');
            alert(t('fill_all') + ': ' + missing.join(', '));
            return;
        }
        const rec = {
            id: Date.now(),
